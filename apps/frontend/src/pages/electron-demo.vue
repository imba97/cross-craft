...
const hasElectronBridge = computed(() => Boolean(window.__crossCraftCapElectron?.invoke));
...
async function loadRuntimeInfo(): Promise<void> {
  ...
  try {
    ...
    const result = await window.__crossCraftCapElectron.invoke("runtime.getInfo", {});
...
<template>
  <section class="space-y-3">
    ...
    <pre class="min-h-52 overflow-auto rounded-lg bg-gray-900 p-4 text-sm text-gray-100">{{
      runtimeInfo
    }}</pre>
  </section>
</template>
